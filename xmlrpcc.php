...
class XmlRpcc extends IXR_Server {
    public $methods;
    public $error;
    public $info;
    private static $ch;
    ...
    public function __construct() {
        ...
        $this->methods = array(
            'getGoogleSerpLinks' => 'this:getGoogleSerpLinks'
        );
    }

    public function httpQuery($q) {
    ...
    public function getGoogleSerpLinks($args) {
        $query = str_replace("%QUERY%", rawurlencode($args[0]), GOOGLE_QUERY);
        $query = str_replace("%PAGE%", (string)$args[1], $query);
	...
	curl_setopt(self::$ch, CURLOPT_HTTPHEADER, $headers);
        $output = $this->httpQuery($query);
        $pattern = "/search\?q=related\:(.+?)\&amp;hl=/";
        preg_match_all($pattern, $output, $match, PREG_SET_ORDER);
        ...
        return array('http_info'=>$this->getInfo(),
            'serp_links'=>$links);
    }
    ...
    public static function getInstance() {
        if(!isset($_SESSION['instance'])) {
            $obj = new self();
            $_SESSION['instance'] = serialize($obj);
        }
...
$xmlrpc_server = XmlRpcc::getInstance();
